...
@Component
@Primary
public class EeaTransactionMonitoringBlockListener extends DefaultTransactionMonitoringBlockListener {
    ...
    public EeaTransactionMonitoringBlockListener(ChainServicesContainer chainServicesContainer, BlockchainEventBroadcaster broadcaster, TransactionDetailsFactory transactionDetailsFactory, BlockCache blockCache, NodeSettings nodeSettings) {
    ...
    protected void broadcastIfMatched(Transaction tx, Block block, List<TransactionMatchingCriteria> criteriaToCheck) {
        ...
        PrivacyConfNode privacyConf = PrivacyUtils.buildPrivacyConfNodeFromExtension(node.getExtension());

        //If node has privacy enabled
        if (privacyConf.isEnabled()) {
            Web3JEeaService eeaInstance = (Web3JEeaService) chainServicesContainer.getNodeServices(block.getNodeName()).getBlockchainService();
            if (eeaInstance.isPrivateTransaction(tx)) {
                Optional.ofNullable(eeaInstance.getPrivateTransactionReceipt(tx.getHash())).ifPresent(txReceipt -> {
                    final TransactionDetails txDetails = transactionDetailsFactory.createTransactionDetails(tx,
                            txReceipt.getStatus().equals("0x1") ? TransactionStatus.CONFIRMED : TransactionStatus.FAILED, block);
                    txDetails.setTo(txReceipt.getTo());
                    txDetails.setRevertReason(node.getAddTransactionRevertReason() ? txReceipt.getRevertReason() : null);
                    criteriaToCheck.stream().filter(matcher -> matcher.isAMatch(txDetails)).findFirst()
