...
public class EeaContractEventProcessor extends DefaultContractEventProcessor {
    ...
    public EeaContractEventProcessor(ChainServicesContainer chainServices, AsyncTaskService asyncTaskService,
    ...
    private void processLogsInNormalNodeBlock(Block block, List<ContractEventFilter> contractEventFilters) {
        ...
        final List<String> privateTransactionsLogsBloom = block.getTransactions().stream()
        ...
        contractEventFilters.forEach(filter -> {
            if (!block.getTransactions().isEmpty() && block.getNodeName().equals(filter.getNode())
                    && ((privateTransactionsLogsBloom.stream().anyMatch(logsBloom -> isEventFilterInBloomFilter(filter, logsBloom)))
    ...
    private void processWeb3LogsForFilter(ContractEventFilter filter, Block block,
