...
public class PrivacyUtils {
    ...
    public static PrivacyConfNode buildPrivacyConfNodeFromExtension(Map <String, Object> extension) {
        ...
        Map<String, Object> subMap =
                (Map<String, Object>) extension.get(PRIVACY_EXTENSION_KEY);

        return subMap == null ?
                null :
                new PrivacyConfNode(Boolean.parseBoolean(subMap.get(PRIVACY_ENABLED_KEY).toString()),
                        (String) subMap.get(PRIVACY_PRECOMPILED_ADDRESS)
                );
    }

    public static PrivacyConfFilter buildPrivacyConfFilterFromExtension(Map<String, Object> extension) {
        if (extension == null){
            return null;
        }
        ...
        return subMap == null ?
                null :
                new PrivacyConfFilter(Boolean.parseBoolean(subMap.get(PRIVACY_ENABLED_KEY).toString()),
                        (String) subMap.get(PRIVACY_GROUP_ID_KEY)
                );
    }

    public static Transaction buildTransactionFromPrivateTransaction(PrivateTransaction privateTransctionReceipt) {
