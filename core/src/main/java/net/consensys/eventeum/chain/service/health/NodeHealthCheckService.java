...
public class NodeHealthCheckService {
  ...
  public NodeHealthCheckService(
    ...
    currentBlock =
        valueMonitor.monitor("currentBlock", blockchainService.getNodeName(), new AtomicLong(0));
    nodeStatusGauge =
    ...
    syncing =
        valueMonitor.monitor("syncing", blockchainService.getNodeName(), new AtomicInteger(0));
  }
  ...
  protected boolean isNodeConnected() {
    try {
      ...
      log.error(
          "Get latest block failed with exception on node " + blockchainService.getNodeName(), t);
