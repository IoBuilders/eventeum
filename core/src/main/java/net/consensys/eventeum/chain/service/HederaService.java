...
public class HederaService implements BlockchainService {
     ...
     */
    private <R> R newCall(Request request, TypeReference<R> typeReference) throws IOException, RuntimeException, NotFoundException {
        ...
        switch (HttpStatus.valueOf(response.code())) {
            ...
            case PARTIAL_CONTENT:
                R responseBody = this.objectMapper.reader().forType(typeReference)
            ...
            case BAD_REQUEST:
                ...
                boolean responseEntityId = bodyText != null && bodyText.toLowerCase().contains("null entity id");
                log.error(message);
                response.close();
                if (responseEntityId) {
                    return null;
     ...
     */
    @SneakyThrows
    private ContractResultsResponse doContractResultsRequest(HttpUrl.Builder httpBuilder, BigInteger tries, BigInteger blockNumber) throws NotFoundException, IOException, MirrorUnexpectedException {
     ...
     */
    public ContractResultResponse getContractResult(String transactionId) throws IOException, NotFoundException {
        String url = nodeUrl + API_VERSION_PATH + CONTRACT_RESULTS_PATH;
        HttpUrl.Builder httpBuilder = HttpUrl.parse(String.format("%s/%s", url, transactionId)).newBuilder();
        Request request = generateHttpRequest(httpBuilder);
     ...
     */
    public ContractResultsResponse getContractResultsByBlock(BigInteger blockNumber) throws IOException, NotFoundException {
        String url = nodeUrl + API_VERSION_PATH + CONTRACT_RESULTS_PATH;
        ...
        ContractResultsResponse response = doContractResultsRequest(httpBuilder, BigInteger.ZERO, blockNumber);

        if (response.getLinks() != null) {
            while (response.getLinks().get(NEXT) != null) {
                httpBuilder = HttpUrl.parse(nodeUrl + response.getLinks().get(NEXT)).newBuilder();
                ContractResultsResponse nestedResponse = doContractResultsRequest(httpBuilder, BigInteger.ZERO, blockNumber);
     ...
     */
    private ContractResultResponse callToContractResult(ContractResultResponse res, BigInteger tries) throws MirrorUnexpectedException, InterruptedException {
        ...
        try {
            ...
            Thread.sleep(500);
            if (tries.compareTo(maxRetries) > 0) {
                String errorMsg = String.format("Max number of retries exceeded when try to recover contract result of %s", res.getHash());
                log.warn(errorMsg);
                throw new MirrorUnexpectedException(errorMsg);
            } else {
    ...
    public List<ContractResultResponse> filterAndGetContractResults(List<ContractResultResponse> responses) throws IOException {
        ...
        List<ContractResultResponse> responsesFiltered = responses.stream().filter(response ->
    ...
    public void processBlock(HederaBlock block) {
        ...
        String ethTimestamp = block.getFromTimestamp().substring(0, block.getFromTimestamp().indexOf("."));
    ...
    public BlockResponse getBlock(BigInteger blockNumber) throws IOException, NotFoundException {
        HttpUrl.Builder httpBuilder = HttpUrl.parse(nodeUrl + API_VERSION_PATH + BLOCKS_PATH).newBuilder();
    ...
    public Flowable<HederaBlock> blocksFlowable(BigInteger startBlock, Long pollingInterval) {
        ...
        return Flowable.create((subscriber) -> scheduledExecutorService.scheduleAtFixedRate(() -> {
            try {
        ...
        }, 0L, pollingInterval, TimeUnit.MILLISECONDS), BackpressureStrategy.DROP);
    }
     ...
     */
    @Override
    public List<ContractEventDetails> retrieveEvents(ContractEventFilter eventFilter, BigInteger startBlock, BigInteger endBlock) {
        ...
        try {
            LogsResponseHederaMirrorNodeResponse logResponse = getLogResponse(eventFilter, startBlock, endBlock);
            ...
            while (logResponse.getLinks().getNext() != null) {
                ...
                Request request = generateHttpRequest(httpBuilder);
                logResponse = this.newCall(request, new TypeReference<LogsResponseHederaMirrorNodeResponse>() {
                });
            ...
            contractEventDetails = logsHederaMirrorNode.stream()
                    .map(el -> eventDetailsFactory.createEventDetails
                            (eventFilter, buildLog(el), BigInteger.ZERO, eventFilter.getContractAddress()))
                    .collect(Collectors.toList());
    ...
    private LogsResponseHederaMirrorNodeResponse getLogResponse(ContractEventFilter eventFilter, BigInteger startBlock, BigInteger endBlock) throws NotFoundException, IOException {
        ...
        HttpUrl.Builder httpBuilder = HttpUrl.parse
                        (nodeUrl + API_VERSION_PATH + LOGS_PATH.replace(CONTRACT_VAR, eventFilter.getContractAddress()))
                .newBuilder();
    ...
    public FilterSubscription registerEventListener(ContractEventFilter eventFilter,
        ...
        try {
            ...
            final BigInteger endBlock = eventStoreService.getLatestBlock(eventFilter.getNode()).orElseThrow(() -> new NotFoundException("Latest block not found")).getNumber();
            Disposable sub = createContractEventFlowable(eventFilter, startBlock, endBlock)
                    .subscribe(eventListener::onEvent, error -> log.error("Error registering event listener", error));
            if (sub.isDisposed()) {
    ...
    public Optional<Block> getBlock(String blockHash, boolean fullTransactionObjects) {
        ...
        HttpUrl.Builder httpBuilder = HttpUrl.parse(String.format("%s/%s", url, blockHash)).newBuilder();
